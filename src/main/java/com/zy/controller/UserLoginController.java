...
public class UserLoginController {
    ...
    public String savePassword(UserLogin userLogin, HttpServletRequest request, HttpServletResponse response) throws Exception {
        JSONObject result = new JSONObject();
        if (userLogin.getPassword().equals(request.getParameter("passwordqueren"))) {
            userLogin.setUserid(Integer.parseInt(request.getParameter("userid")));
     ...
     * @throws Exception
     */
    @RequestMapping("/getAchieveCode")
    public String getAchieveCode(HttpServletRequest request,HttpServletResponse response) throws Exception {
        Mail mail=new Mail();
        /*生成随机码*/
        ...
        /*偷懒,只设置了一个管理员,所以只判断了userid是否为1,是1则为管理员登录,调用管理员的方法;不为1,则调用教师方法*/
        if(Integer.parseInt(request.getParameter("userid"))==1){
            Admin admin=userLoginService.getEmailAdmin(Integer.parseInt(request.getParameter("userid")));
            /*把获取到的邮箱和随机码传进去*/
            mail.sendMail(admin.getEmail(),randomArise);
        }else{
            Teacher teacher=userLoginService.getEmailTeacher(Integer.parseInt(request.getParameter("userid")));
     ...
     * @throws Exception
     */
    @RequestMapping("/updatePasswordAdminAndTeacher")
    public String savePasswordAdmin(UserLogin userLogin, HttpServletRequest request, HttpServletResponse response) throws Exception {
        JSONObject result = new JSONObject();
        /*判断密码和验证码是否相同*/
        if (userLogin.getPassword().equals(request.getParameter("passwordqueren"))) {
            /*判断验证码是否正确*/
            if(map.get("key").equals(Integer.parseInt(request.getParameter("code")))){
                userLogin.setUserid(Integer.parseInt(request.getParameter("userid")));
                int resultTotal = userLoginService.updatePassword(userLogin);
                result.put("success", true);
                ResponseUtil.write(response, result);
